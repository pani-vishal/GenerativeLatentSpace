...
class InducedMetricNF:
    ...
    def curveLength(self, dt, curve_points, curve_derivatives=None, M_batch_size=4):
        ...
        if curve_derivatives is None:
            ...
            length = np.sqrt(np.matmul(np.matmul(z_diff.reshape(N - 1, 1, d), M), z_diff.reshape(N - 1, d, 1)).reshape(
                -1) + 1e-6).sum()

        else:
            M = self.M_valueAt(pt.Tensor(curve_points).to(self.device), M_batch_size=M_batch_size)
            length = dt * np.sqrt(
                np.matmul(np.matmul(curve_derivatives.reshape(N, 1, d), M), curve_derivatives.reshape(N, d, 1)).reshape(
                    -1) + 1e-6).sum()
    ...
    def M_valueAt(self, z, M_batch_size=None):
        ...
        X_pred, _ = self.modelG.to_image(z_J[:, None, :])
        X_pred = pt.squeeze(X_pred, axis=1)
